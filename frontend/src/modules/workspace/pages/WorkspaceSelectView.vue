...
<template>
  <div class="w-full min-w-screen mx-4 md:mt-24">
    ...
    <div class="text-center mb-12">
      ...
      <p class="text-text/60 mt-2 max-w-md mx-auto">Choose your destination for this session. You can change this later from the top navigation.</p>
    </div>
    ...
    <div class="flex justify-center gap-7 flex-wrap mx-4">
      <template v-for="role in roles" :key="role">
        <div
          v-if="currentWorkspace[role]"
          class="max-w-100 group relative bg-surface border border-border rounded-2xl p-7 flex flex-col shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
          <!-- Overflow menu -->
          <div class="absolute top-5 right-5">
            <button
              type="button"
              @click.stop="openMenuId = openMenuId === role ? null : role"
            ...
            <div v-if="openMenuId === role" class="absolute right-0 mt-1 w-48 bg-surface border border-border rounded-lg shadow-lg py-1 z-10">
              <button type="button" @click="setDefault(role)" class="w-full flex items-center gap-2 px-3 py-2 text-sm text-text hover:bg-bg transition-colors text-left">
                <Star class="w-4 h-4" :class="defaultWorkspaceId === role ? 'text-accent' : 'text-text/40'" />
          ...
          <!-- Icon -->
          <div class="w-12 h-12 rounded-xl flex items-center justify-center mb-5" :class="workspaces[role].iconBg">
          ...
          <div class="flex items-center gap-2 mb-2 pr-6">
            ...
            <span v-if="defaultWorkspaceId === role" class="text-[11px] font-semibold text-accent bg-accent/10 px-2 py-0.5 rounded-full"> Default </span>
          </div>
          ...
          <BaseButton variant="primary" block class="mt-6" @click="handleWorkspace(role)">
            <RouterLink :to="role + '/dashboard'"> Enter Workspace </RouterLink>
            <template #icon>
  ...
  <div class="text-center text-sm text-text/50 mb-10">
    ...
    <div class="flex items-center justify-center gap-2 mt-1">
      <router-link to="/privacy" class="font-semibold text-text/70 hover:text-accent transition-colors"> Privacy Policy </router-link>
      <span class="w-1 h-1 rounded-full bg-text/30" />
      <router-link to="/terms" class="font-semibold text-text/70 hover:text-accent transition-colors"> Terms of Service </router-link>
      <span class="w-1 h-1 rounded-full bg-text/30" />
      <router-link to="/support" class="font-semibold text-text/70 hover:text-accent transition-colors"> Support </router-link>
    </div>
  </div>
</template>
