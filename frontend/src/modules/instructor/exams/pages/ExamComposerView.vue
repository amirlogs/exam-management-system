...
async function loadBank() {
    const res = await questionsApi.getQuestions({ content: search.value }, 1)
    bank.value = res.data.filter(q => !attached.value.some(a => a.id === q.id))
}
onMounted(async () => { await load(); await loadBank() })
...
function goImport() {
    router.push(`/instructor/imports/new?examId=${examId.value}${exam.value ? '&courseId=' + route.query.courseId : ''}`)
}
    ...
    <div v-if="exam" class="mx-auto w-full max-w-360 space-y-6 px-6 py-8 min-h-[calc(100vh-68px)]">
        ...
        <div class="flex flex-wrap gap-3 bg-surface border border-border rounded-xl p-4">
            ...
            <div class="flex items-center gap-2 bg-bg border border-border rounded-full px-4 py-2">
                ...
                <span class="text-xs text-text/60">· {{ exam.composition.true_false?.marks_each ?? 0 }} marks
                    each</span>
                <button v-if="canEdit" class="text-text/40 hover:text-accent" @click="openCompositionEdit">
                    <Pencil class="w-3.5 h-3.5" />
                </button>
            ...
            <div v-if="editingComposition" class="w-full flex items-end gap-3 pt-2 border-t border-border mt-2">
                ...
                <BaseInput v-model.number="compositionDraft.true_false" type="number" step="0.5"
                    label="True/False marks each" />
            ...
            <div class="lg:col-span-8">
                <BaseCard :padded="false" class="rounded-xl border border-border overflow-hidden shadow-sm">
                    <div class="flex border-b border-border">
                        <button class="flex-1 py-3.5 text-sm font-semibold border-b-2 transition-colors"
                            :class="activeTab === 'bank' ? 'text-accent border-accent' : 'text-text/50 border-transparent'"
                            @click="activeTab = 'bank'">Add from Bank</button>
                        <button class="flex-1 py-3.5 text-sm font-semibold border-b-2 transition-colors"
                            :class="activeTab === 'import' ? 'text-accent border-accent' : 'text-text/50 border-transparent'"
                            @click="activeTab = 'import'">Import</button>
                    </div>

                    <div v-if="activeTab === 'bank'" class="p-5">
                        <input v-model="search" placeholder="Search questions…"
                            class="w-full px-3 py-2.5 rounded-lg border border-border bg-bg text-sm mb-4"
                            @input="loadBank" />
                        <div class="divide-y divide-border max-h-[420px] overflow-y-auto">
                            <div v-for="q in bank" :key="q.id" class="py-3 flex items-center justify-between gap-3">
                                <div class="min-w-0 flex-1">
                                    ...
                                    <BaseBadge variant="neutral" class="uppercase text-[10px] mt-1">{{ q.type }}
                                    </BaseBadge>
                                </div>
                                <input v-if="q.type !== 'mcq' && q.type !== 'true_false'"
                                    v-model.number="manualMarks[q.id]" type="number" step="0.5" placeholder="Marks"
                                    class="w-20 px-2 py-1.5 rounded-md border border-border text-sm" />
                                <BaseButton size="sm" variant="secondary" :disabled="!canEdit" @click="addFromBank(q)">
                                    Add</BaseButton>
                            </div>
                            <p v-if="!bank.length" class="text-sm text-text/40 text-center py-8">No available questions.
                            </p>
                        </div>
                    </div>

                    <div v-else class="p-10 text-center">
                        <p class="text-sm text-text/60 mb-4">Import a CSV of questions directly into this exam.</p>
                ...
                <BaseCard class="sticky top-6">
                    ...
                    <div class="space-y-5 max-h-[420px] overflow-y-auto">
                        <div v-for="(qs, type) in attachedByType" :key="type">
                            <h4 class="text-xs font-bold uppercase tracking-wide text-text/50 mb-2">{{ type }} ({{
                                qs.length }})</h4>
                            <div class="space-y-1.5">
                                <div v-for="q in qs" :key="q.id"
                                    class="flex items-center justify-between gap-2 bg-bg border border-border rounded-lg px-3 py-2">
                                    <span class="text-sm text-text truncate">{{ q.content }}</span>
                                    <button v-if="canEdit" class="text-text/40 hover:text-red-500 shrink-0"
                        ...
                        <p v-if="!attached.length" class="text-sm text-text/40 text-center py-8">No questions attached
                            yet.</p>
                    </div>
                    <div class="mt-4 pt-4 border-t border-border">
                        <div
                            class="bg-accent/10 text-accent rounded-lg px-4 py-3 flex justify-between items-center font-semibold text-sm">
