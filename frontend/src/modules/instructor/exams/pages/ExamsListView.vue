...
function openCreate() {
  form.value = { title: '', type: 'MIDTERM', duration_minutes: 60, mcq_marks: 1, tf_marks: 1 };
...
<template>
  <div class="mx-auto w-full max-w-360 space-y-6 px-6 py-8 min-h-[calc(100vh-68px)]">
    <ResourceToolbar title="Exams" description="Manage exams for this course offering." show-refresh :refreshing="refreshing" @refresh="handleRefresh">
      <template #actions>
      ...
      <table class="w-full">
        <thead>
          <tr class="bg-bg/40 border-b border-border">
            <th class="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-text/45">Title</th>
            <th class="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-text/45">Type</th>
            <th class="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-text/45">Questions</th>
        ...
        <tbody v-else-if="exams.length" class="divide-y divide-border">
          <tr v-for="exam in exams" :key="exam.id" class="cursor-pointer hover:bg-bg/40 transition-colors" @click="openExam(exam)">
            <td class="px-6 py-5 text-sm font-medium text-text">{{ exam.title || '—' }}</td>
            <td class="px-6 py-5 text-sm text-text/70">{{ exam.type }}</td>
            <td class="px-6 py-5 text-sm text-text/70">{{ exam.total_questions ?? 0 }} · {{ exam.total_marks ?? 0 }} marks</td>
            <td class="px-6 py-5">
