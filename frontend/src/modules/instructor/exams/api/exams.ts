...
export function getExams(courseOfferingId: number, page = 1, filters: Record<string, any> = {}) {
  return apiClient
    .get(`/course-offerings/${courseOfferingId}/exams`, { params: { page, ...filters } })
    .then(unwrapPaginated<import('../types/exam').Exam>)
}
...
export function createExam(
  ...
  payload: { title: string; type: string; duration_minutes: number; composition: Record<string, { marks_each?: number }> },
) {
