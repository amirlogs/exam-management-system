...
export function useQuestionBankImport(courseId: Ref<number>) {
  ...
  async function updateRow(rowIndex: number, data: QuestionRowData) {
    ...
    try {
      importRecord.value = await api.updateImportRow(importRecord.value.import_id, rowIndex, data)
    } finally {
  ...
  async function deleteRow(rowIndex: number) {
    ...
    isSavingRow.value = true
    try {
      importRecord.value = await api.deleteImportRow(importRecord.value.import_id, rowIndex)
    } finally {
      isSavingRow.value = false
    }
  }
  ...
  async function confirm() {
    ...
    try {
      importRecord.value = await api.confirmImport(importRecord.value.import_id)
    } finally {
