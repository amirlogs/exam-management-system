...
async function handleRefresh() { refreshing.value = true; await load(); refreshing.value = false }
watch(search, load)
onMounted(load)
...
<template>
    <div class="mx-auto w-full max-w-360 space-y-6 px-6 py-8 min-h-[calc(100vh-68px)]">
        <ResourceToolbar title="My Courses" description="Course offerings you're assigned to teach."
            search-placeholder="Search courses…" show-search show-refresh :refreshing="refreshing"
            @update:search="v => search = v" @refresh="handleRefresh" />
        ...
        <div v-else-if="offerings.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="offering in offerings" :key="offering.id"
                ...
                <div class="flex items-start justify-between mb-4">
                    ...
                    <BaseBadge :variant="statusVariant[offering.status] || 'neutral'" class="text-[10px]">{{
                        offering.status.replace('_', ' ') }}</BaseBadge>
                </div>
                <h3 class="font-semibold text-text">{{ offering.course?.name || '—' }}</h3>
                <p class="text-sm text-text/50 mt-0.5">{{ offering.course?.code || '—' }} · {{ offering.semester?.name
                    || '—' }}</p>
            </div>
        </div>

        <div v-else class="bg-surface border border-border rounded-xl p-16 text-center text-sm text-text/50">
