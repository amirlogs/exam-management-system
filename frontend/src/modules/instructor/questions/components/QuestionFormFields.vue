...
function removeOption(i: number) {
  props.form.options.splice(i, 1);
}
function setTrueFalse() {
  props.form.options = ['True', 'False'];
}
</script>
...
<template>
  <div class="space-y-4">
    ...
    <div>
      ...
      <input v-model="form.chapter" class="w-full px-3 py-2.5 rounded-lg border border-border bg-bg text-sm" placeholder="e.g. Propositional Logic" />
    </div>
    <div>
    ...
    <div v-if="isChoiceType" class="space-y-3">
      <div class="flex items-center justify-between">
        ...
        <button v-if="form.type === 'mcq'" type="button" class="inline-flex items-center gap-1 text-xs text-accent font-semibold" @click="addOption">
          <Plus class="h-3.5 w-3.5" /> Add option
        </button>
      </div>
      <div v-for="(_, i) in form.options" :key="i" class="flex items-center gap-2">
        <input type="radio" :checked="form.correct_answer === form.options[i]" class="accent-accent" @change="form.correct_answer = form.options[i]" />
        <input v-model="form.options[i]" :disabled="form.type === 'true_false'" class="flex-1 px-3 py-1.5 rounded-md border border-border bg-bg text-sm disabled:opacity-60" />
        <button v-if="form.type === 'mcq' && form.options.length > 2" type="button" class="text-text/40 hover:text-red-500" @click="removeOption(i)">
