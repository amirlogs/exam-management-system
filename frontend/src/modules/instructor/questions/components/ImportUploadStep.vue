<!-- frontend/src/modules/admin/imports/components/ImportUploadStep.vue -->
...
const props = withDefaults(
  defineProps<{
    ...
    uploading: boolean;
    error: string | null;
    /** Override the course options instead of fetching from the admin /courses endpoint (e.g. scope to an instructor's teaching courses). */
    courseOptions?: { value: string; label: string }[] | null;
    /** Pre-fill context fields, e.g. { course_id: '3', exam_id: '12' }. */
...
const context = reactive<Record<string, any>>({ ...props.initialContext });
...
watch(
  ...
  () => {
    ...
    Object.assign(context, props.initialContext);
...
onMounted(async () => {
  ...
  if (config.value.contextFields.some((field) => field.kind === 'course_select')) {
    if (props.courseOptions) {
      courseOptions.value = props.courseOptions;
    } else {
