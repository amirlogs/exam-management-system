...
async function load(page = 1) {
  ...
  try {
    ...
    const res = activeView.value === 'archived' ? await questionsApi.getArchivedQuestions(filters, page) : await questionsApi.getQuestions(filters, page);
...
function openEdit(q: Question) {
  ...
  reset({
    ...
    correct_answer: q.options.find((o) => o.is_correct)?.option_text || '',
  });
  showFormModal.value = true;
}
...
function openArchive(q: Question) {
  selected.value = q;
...
<template>
  <div class="mx-auto w-full max-w-360 space-y-6 px-6 py-8 min-h-[calc(100vh-68px)]">
    <ResourceToolbar
      title="Question Bank"
      description="Manage and curate assessment content for this course."
      ...
      <template #filters>
        <div class="grid grid-cols-3 gap-3">
          ...
          <input v-model="filterChapter" placeholder="Chapter…" class="px-3 py-2.5 rounded-lg border border-border bg-surface text-sm" @change="load(1)" />
      ...
      <table class="w-full border-collapse">
        <thead>
          <tr class="bg-bg/40 border-b border-border">
            ...
            <th class="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-text/45">Question Content</th>
            <th class="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-text/45">Chapter</th>
  ...
  <BaseDialog :model-value="showFormModal" :title="selected ? 'Edit question' : 'Create question'" @update:model-value="closeFormModal">
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        ...
        <BaseButton :loading="saving" @click="submitForm">{{ selected ? 'Save changes' : 'Create question' }} </BaseButton>
      </div>
    </template>
  </BaseDialog>
