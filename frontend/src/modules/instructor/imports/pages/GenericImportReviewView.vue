...
function cancelEdit() { editingRow.value = null; draft.value = null }
...
onMounted(() => { if (importId.value) start(importId.value) })
</script>
...
<template>
    <div class="mx-auto w-full max-w-360 space-y-6 px-6 py-8 min-h-[calc(100vh-68px)]">
        <div>
            ...
            <div class="flex flex-wrap items-center gap-2 mt-2">
                <span v-if="courseId"
                    class="inline-flex items-center px-3 py-1 bg-bg border border-border rounded-full text-sm text-text/70">Course
                    #{{ courseId }}</span>
                <span v-if="examId"
                    class="inline-flex items-center px-3 py-1 bg-bg border border-border rounded-full text-sm text-text/70">Exam
                    #{{ examId }}</span>
            </div>
        </div>

        <!-- No import started yet -->
        <div v-if="!importRecord"
            class="bg-surface border-2 border-dashed border-border rounded-xl p-12 flex flex-col items-center text-center">
            ...
            <div v-else class="flex items-center gap-3">
                ...
                <BaseButton :disabled="isUploading" @click="handleUpload">{{ isUploading ? 'Uploading…' : 'Upload' }}
                </BaseButton>
            </div>
        ...
        <!-- Processing -->
        <div v-else-if="importRecord.status === 'pending'"
            class="bg-surface border border-border rounded-xl p-12 flex flex-col items-center text-center">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-bg/40 border-b border-border">
                            <th
                                class="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-text/45 w-16">
                                Row</th>
                            <th class="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-text/45">
                                Content</th>
                            <th
                                class="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-text/45 w-32">
                                Type</th>
                            <th
                                class="px-6 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-text/45 w-32">
                                Status</th>
                    ...
                    <tbody class="divide-y divide-border">
                        <template v-for="[rowNum, row] in rowEntries" :key="rowNum">
                            <tr :class="row.status === 'invalid' ? 'bg-red-500/5' : ''">
                                ...
                                <td class="px-6 py-4 text-sm text-text max-w-md truncate">{{ row.data.content || '—' }}
                                </td>
                                <td class="px-6 py-4 text-sm text-text/70 capitalize">{{ row.data.type }}</td>
                                <td class="px-6 py-4">
                                    <span v-if="row.status === 'valid'"
                                        class="inline-flex items-center gap-1 text-emerald-600 text-xs font-semibold">
                                        <CheckCircle2 class="w-3.5 h-3.5" /> Valid
                                    </span>
                                    <span v-else
                                        class="inline-flex items-center gap-1 text-red-600 text-xs font-semibold">
                                        <XCircle class="w-3.5 h-3.5" /> {{ row.errors.length }} issue(s)
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-1">
                                        <button class="p-1.5 text-text/50 hover:text-accent"
                                            @click="startEdit(rowNum, row)">
                                            <Pencil class="w-4 h-4" />
                                        </button>
                                        <button class="p-1.5 text-text/50 hover:text-red-500"
                                ...
                                <td colspan="5" class="px-6 py-4">
                                    <div class="bg-surface p-4 rounded-lg border border-accent/30 space-y-3">
                                        ...
                                        <div class="flex justify-end gap-2 pt-2 border-t border-border">
                                            ...
                                            <BaseButton :disabled="isSaving" @click="saveRow(rowNum)">{{ isSaving ?
                                                'Saving…' : 'Save Row' }}</BaseButton>
                                        </div>
                                    </div>
                                </td>
                            </tr>
        ...
        <!-- Confirmed -->
        <div v-else-if="importRecord.status === 'confirmed'"
            class="bg-surface border border-border rounded-xl p-12 flex flex-col items-center text-center">
    ...
    <ConfirmModal :show="showCancelModal" title="Cancel this import?"
        ...
        confirm-text="Cancel Import" variant="danger" :icon="X" :loading="isCancelling" @close="showCancelModal = false"
