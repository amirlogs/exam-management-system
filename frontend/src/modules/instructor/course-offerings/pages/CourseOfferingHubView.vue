...
const sections = computed(() => [
    {
        label: 'Question Bank', desc: 'Create, import, and manage this course\'s questions.', icon: HelpCircle,
        to: `/instructor/courses/${courseOfferingId.value}/questions`,
    },
    {
        label: 'Exams', desc: 'Build, submit, and manage exams for this course offering.', icon: FileText,
...
<template>
    ...
    <div v-else class="mx-auto w-full max-w-360 space-y-6 px-6 py-8 min-h-[calc(100vh-68px)]">
        <div>
            ...
            <p class="text-sm text-text/60 mt-1">{{ offering?.course?.code || '—' }} · {{ offering?.semester?.name ||
                '—' }}</p>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <router-link v-for="s in sections" :key="s.label" :to="s.to"
                class="bg-surface border border-border rounded-xl p-6 hover:border-accent hover:shadow-sm transition-all">
