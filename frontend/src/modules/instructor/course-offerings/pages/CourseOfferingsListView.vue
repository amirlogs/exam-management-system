...
<template>
  <div class="mx-auto w-full max-w-360 space-y-6 px-6 py-8 min-h-[calc(100vh-68px)]">
    <ResourceToolbar
      title="Exams"
      description="Select a course offering to manage its exams."
    ...
    <div class="rounded-xl border border-border bg-surface overflow-hidden shadow-sm">
      <table class="w-full border-collapse">
        ...
        <tbody v-else-if="offerings.length" class="divide-y divide-border">
          <tr v-for="offering in offerings" :key="offering.id" class="cursor-pointer hover:bg-bg/40 transition-colors" @click="openOfferingExams(offering)">
            <td class="px-6 py-5">
        ...
        <tbody v-else>
          <tr>
            <td colspan="5" class="px-6 py-16 text-center text-sm text-text/50">No course offerings assigned yet.</td>
          </tr>
        </tbody>
      </table>
