...
import { Building2, Edit, MapPin, Hash } from 'lucide-vue-next'
...
async function loadUniversity() {
    ...
    error.value = null
    try {
        // ASSUMPTION: no dedicated "current university" endpoint yet — fetching page 1, 1 item.
        // Swap this for a real GET /university call if the backend adds a singleton route later.
...
function openEdit() {
    if (university.value) {
        reset({ name: university.value.name, code: university.value.code, address: university.value.address })
    } else {
...
async function submitForm() {
    ...
    try {
        if (university.value) {
            university.value = await updateUniversity(university.value.id, form)
        } else {
            university.value = await createUniversity(form)
        }
        closeFormModal()
        saving.value = false
...
<template>
    <div class="mx-auto w-full max-w-160 space-y-6 px-6 py-6">
        <section>
            <h1 class="text-2xl font-semibold tracking-tight text-text">Institution Settings</h1>
            <p class="mt-1 text-sm text-text/60">Manage your university's basic information.</p>
        </section>
        ...
        <!-- Error -->
        <div v-else-if="error" class="flex items-start gap-3 rounded-md border border-error/30 bg-error/5 p-4">
            <p class="flex-1 text-sm font-medium text-error">{{ error }}</p>
            <button type="button" class="text-sm font-medium text-error hover:underline" @click="loadUniversity">Retry</button>
        </div>

        <!-- Bootstrap: no university set up yet -->
        <BaseCard v-else-if="!university">
            <div class="flex flex-col items-center py-6 text-center">
                ...
                <p class="mt-1 max-w-sm text-sm text-text/55">
                    Add your institution's name, code, and address to get started.
                </p>
        ...
        <!-- Normal: show current info -->
        <BaseCard v-else>
            <div class="flex items-start justify-between gap-4">
                <div class="flex items-center gap-3">
                    <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-md border border-border bg-bg text-sm font-semibold text-text">
    ...
    <BaseDialog :model-value="showFormModal" :title="university ? 'Edit university details' : 'Set up your university'" @update:model-value="closeFormModal">
        <div class="space-y-4">
            <BaseInput v-model="form.name" label="University name" placeholder="e.g. Addis Ababa University" :error="errors.name" />
            <BaseInput v-model="form.code" label="University code" placeholder="e.g. AAU" :error="errors.code" />
            <BaseInput v-model="form.address" label="Address" placeholder="University address" :error="errors.address" />
        </div>
        <template #footer>
            <div class="flex justify-end gap-2">
                <BaseButton variant="secondary" @click="closeFormModal">Cancel</BaseButton>
                <BaseButton :loading="saving" @click="submitForm">{{ university ? 'Save changes' : 'Create university' }}</BaseButton>
            </div>
        </template>
    </BaseDialog>
</template>
