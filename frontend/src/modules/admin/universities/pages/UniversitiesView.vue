...
import { Building2, Edit, MapPin, Hash, RefreshCw, Maximize2, Minimize2 } from 'lucide-vue-next';
...
async function submitForm() {
  ...
  try {
    university.value = university.value ? await updateUniversity(university.value.id, form) : await createUniversity(form);
    closeFormModal();
    saving.value = false;
...
<template>
  <div class="mx-auto w-full max-w-160 space-y-6 px-6 py-8 min-h-[calc(100vh-68px)]">
    ...
    <BaseCard v-else-if="!university">
      <div v-if="permissionsStore.hasPermission('university.create')" class="flex flex-col items-center py-6 text-center">
        ...
        <p class="mt-1 max-w-sm text-sm text-text/55">Add your institution's name, code, and address to get started.</p>
