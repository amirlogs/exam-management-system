...
const search = ref('')
const statusFilter = ref<OfferingStatus | null>(
    null,
)
...
const creatingCourseId = ref<number | null>(
    null,
)
...
const detailOffering = ref<CourseOffering | null>(
    null,
)
...
const selectedOffering = ref<CourseOffering | null>(
    null,
)
...
async function createFromSuggestion(
    ...
    try {
        ...
        uiStore.showToast(
            `${suggestion.course_code} offering created successfully.`,
            'success',
        )
...
async function openDetail(
    ...
    try {
        detailOffering.value =
            await getOffering(offering.id)
...
async function confirmArchive() {
    ...
    try {
        ...
        uiStore.showToast(
            'Course offering archived successfully.',
            'success',
        )
        ...
        uiStore.showToast(
            err?.response?.data?.message ||
            'Failed to archive course offering.',
            'error',
        )
    } finally {
...
async function confirmRestore() {
    ...
    try {
        await restoreOffering(
            selectedOffering.value.id,
        )

        uiStore.showToast(
            'Course offering restored successfully.',
            'success',
        )
        ...
        uiStore.showToast(
            err?.response?.data?.message ||
            'Failed to restore course offering.',
            'error',
        )
    } finally {
...
<template>
    <div class="mx-auto w-full max-w-360 space-y-6 px-6 py-6">
        ...
        <div data-columns-container class="relative">
            <ResourceToolbar title="Course offerings" description="Review and manage course offerings for the semester."
                ...
                :active-tab="activeTab" :active-count="activeTab === 'active'
                        ? pagination.total
                        : undefined
                    " :archived-count="activeTab === 'archived'
                        ? pagination.total
                        : undefined
                    " :has-active-filters="hasActiveFilters
                    " :refreshing="loading" @clear-filters="clearFilters" @change-tab="changeTab"
                @update:search="search = $event" @refresh="retry" @columns="
                    showColumns = !showColumns
                    ">
                <template #actions>
                    <BaseSelect :model-value="String(
                        selectedSemesterId ??
                        '',
                    )
                        " :options="semesterOptions" placeholder="Select semester" @update:model-value="
                            onSemesterChange
                        " />
                ...
                <div class="space-y-0.5">
                    <button v-for="column in columns" :key="column.key" type="button"
                        ...
                        <span class="flex h-4 w-4 shrink-0 items-center justify-center rounded border transition-colors"
                            ...
                            " viewBox="0 0 12 12" class="h-3 w-3" fill="none" stroke="currentColor"
                                stroke-width="2">
        ...
        <!-- Suggestions -->
        <SuggestionsPanel v-if="
            showSuggestions &&
            activeTab === 'active'
        " :suggestions="suggestions" :creating-key="creatingCourseId
                    ? String(creatingCourseId)
                    : null
                " @create="createFromSuggestion" @close="
            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-190 border-collapse">
                    ...
                    <tbody v-if="loading" class="divide-y divide-border">
                        <tr v-for="row in 6" :key="row">
                            <td v-for="column in visibleColumns" :key="column" class="px-4 py-4">
                                <div class="h-3.5 animate-pulse rounded bg-text/5" :class="column ===
                                        'course'
                                        ? 'w-44'
                                        : 'w-20'
                                    " />
                    ...
                    " class="divide-y divide-border">
                        <tr v-for="offering in filteredList" :key="offering.id"
                            class="group cursor-pointer transition-colors hover:bg-text/2" @click="
                                openDetail(
                                    offering,
                                )
                                ">
                            ...
                            <td class="px-4 py-3.5 text-right" @click.stop>
                                <TableRowActions :active-tab="activeTab
                                    " archive-permission="course_offering.archive"
                                    restore-permission="course_offering.restore" :extra-actions="[
                                        {
                                            key: 'view',
                                            label: 'View',
                                            icon: Eye,
                            ...
                            <td :colspan="totalTableColumns
                                ...
                                <div class="mx-auto flex max-w-sm flex-col items-center">
                                    ...
                                    <BaseButton v-if="
                                        ...
                                        'active'
                                    " v-can="'course_offering.create'
                                            " variant="secondary" class="mt-4" @click="
                                            openSuggestions
                                        ">
                                        <template #icon>
                                            <Sparkles class="h-4 w-4" />
    ...
    <!-- Archive -->
    <ConfirmModal :show="showArchiveModal" title="Archive this offering?" :description="`This will remove the ${selectedOffering?.course?.code ?? 'course'} offering from the active list.`
        " confirm-text="Archive offering" variant="danger" :icon="ArchiveIcon" :loading="archiving" @close="
            showArchiveModal = false
            " @confirm="confirmArchive" />

    <!-- Restore -->
    <ConfirmModal :show="showRestoreModal" title="Restore this offering?" :description="`This will return the ${selectedOffering?.course?.code ?? 'course'} offering to the active list.`
        " confirm-text="Restore offering" variant="accent" :icon="RotateCcw" :loading="restoring" @close="
