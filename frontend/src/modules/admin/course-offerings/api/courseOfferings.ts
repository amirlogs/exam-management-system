...
import type { CourseOffering, OfferingSuggestion, OfferingStatus, OfferingUser } from '../types/courseOffering'
...
): Promise<ListResponse<CourseOffering>> {
  const res = await api.get('/course-offerings', {
    params: { semester_id: semesterId, page, per_page: perPage, status },
  })
  return res.data
}

export async function getArchivedOfferings(
  ...
  const res = await api.get('/course-offerings/archived', {
    params: { semester_id: semesterId ?? undefined, page, per_page: perPage, status },
  })
  return res.data
}
...
export async function attachInstructor(
  offeringId: number,
  instructorId: number,
...
// =================== users ======================
export async function getUsers(page = 1, perPage = 200): Promise<ListResponse<OfferingUser>> {
