  ...
  <div class="rounded-md border border-accent/30 bg-accent/5 p-5">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div class="flex items-start gap-2.5">
        ...
        <div>
          <h3 class="font-semibold text-text">{{ grouped.length }} course offerings suggested</h3>

          <p class="mt-0.5 text-xs text-text/50">Suggestions come from active curricula for the selected semester.</p>
        </div>
      </div>

      <button type="button" class="rounded-md p-1 text-text/40 transition-colors hover:bg-text/5 hover:text-text" @click="emit('close')">
        <X class="h-4 w-4" />
      </button>
    </div>

    <div v-if="!grouped.length" class="rounded-md border border-dashed border-border py-8 text-center text-sm text-text/50">
        ...
        <div class="flex items-start justify-between gap-4 p-4">
          <div class="min-w-0">
            ...
            <div class="mt-3 flex flex-wrap gap-1.5">
              <span
                v-for="target in course.targets"
                :key="`${target.program_id}-${target.year_level}`"
          ...
          <BaseButton
            ...
            variant="secondary"
            :loading="creatingKey === key(course.course_id)"
            @click="
              emit('create', {
