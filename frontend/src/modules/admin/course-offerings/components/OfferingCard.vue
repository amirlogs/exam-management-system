...
<template>
    <div class="cursor-pointer rounded-md border bg-surface p-4 transition-colors hover:border-accent" :class="[
        offering.status === 'cancelled' ? 'opacity-50 border-border' : 'border-border',
        offering.status === 'rejected' ? 'border-l-4 border-l-error' : '',
    ]" @click="$emit('click')">
        ...
        <div v-if="offering.status !== 'cancelled'"
            class="flex items-center gap-4 border-y border-border/50 py-2 text-xs">
        ...
        <p v-if="offering.status === 'rejected' && offering.rejection_reason"
