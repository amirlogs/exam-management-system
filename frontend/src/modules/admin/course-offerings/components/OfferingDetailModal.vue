...
const instructorOptions = computed(() =>
    instructors.value
        ...
        .map((instructor) => ({
            ...
            label: `${instructor.user?.first_name ?? ''} ${instructor.user?.last_name ?? ''}`.trim()
                || instructor.employee_number,
        })),
)

const assignmentSection = ref<number | null>(null)
...
async function refresh(offeringId: number) {
    ...
    try {
        ...
        uiStore.showToast('Failed to load course offering.', 'error')
        close()
    } finally {
        loading.value = false
    }
...
function startAssignment(sectionId: number) {
    assignmentSection.value = sectionId
    selectedInstructorId.value = ''
    instructorType.value = 'lead_instructor'
}

function cancelAssignment() {
    assignmentSection.value = null
    selectedInstructorId.value = ''
}
...
async function removeAssignment(assignment: InstructorAssignment) {
    ...
    try {
        ...
        uiStore.showToast(
            err?.response?.data?.message || 'Failed to remove instructor.',
            'error',
        )
    } finally {
        busy.value = false
...
async function changeAssignment(
    ...
    const nextInstructorId = window.prompt(
        'Enter the instructor ID to assign',
    ...
    try {
        await updateInstructorAssignment(
            ...
            assignment.id,
            {
                instructor_id: Number(nextInstructorId),
            },
        )
        ...
        uiStore.showToast('Instructor assignment updated.', 'success')
    } catch (err: any) {
        uiStore.showToast(
...
async function approve() {
    ...
    try {
        ...
        uiStore.showToast(
            err?.response?.data?.message || 'Failed to approve offering.',
            'error',
        )
    } finally {
        busy.value = false
...
async function confirmReject() {
    ...
    try {
        ...
        uiStore.showToast(
            err?.response?.data?.message || 'Failed to reject offering.',
            'error',
        )
    } finally {
        busy.value = false
...
async function confirmCancel() {
    ...
    try {
        ...
        uiStore.showToast(
            err?.response?.data?.message || 'Failed to cancel offering.',
            'error',
        )
    } finally {
        busy.value = false
...
async function reopen() {
    ...
    try {
        ...
        uiStore.showToast(
            err?.response?.data?.message || 'Failed to reopen offering.',
            'error',
        )
    } finally {
...
async function confirmArchive() {
    ...
    try {
        ...
        uiStore.showToast(
            err?.response?.data?.message || 'Failed to archive offering.',
            'error',
        )
    } finally {
...
function typeLabel(type: InstructorAssignmentType) {
...
<template>
    <BaseDialog :model-value="modelValue" :title="local?.course?.name" :description="local?.course?.code"
        ...
        <div v-else-if="local" class="space-y-6">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-xs text-text/50">
                        {{ local.semester?.name }} · {{ local.semester?.academic_year }}
                    </p>
                </div>
            ...
            <!-- Rejection -->
            <div v-if="local.status === 'rejected' && local.rejection_reason"
            ...
            <section>
                <div class="mb-3 flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-text/45">
                            These sections were determined automatically from the active curriculum.
                        </p>
                    </div>
                ...
                <div class="space-y-2">
                    <div v-for="section in local.sections" :key="section.id"
                        class="rounded-md border border-border bg-bg/40 p-3">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div>
                            ...
                            <BaseButton v-if="isDraft" v-can="'course_offering.assign_instructor'" variant="secondary"
                                size="sm" @click="startAssignment(section.id)">
                                <template #icon>
                        ...
                        <div class="mt-3 space-y-2">
                            <div v-for="assignment in assignmentForSection.get(section.id) ?? []" :key="assignment.id"
                                ...
                                <div class="flex shrink-0 items-center gap-1">
                                    <BaseButton v-if="isDraft" v-can="'course_offering.change_instructor'"
                                        variant="ghost" size="sm" @click="changeAssignment(assignment)">
                                        Change
                                    </BaseButton>

                                    <BaseButton v-if="isDraft" v-can="'course_offering.remove_instructor'"
                                        variant="ghost" size="sm" class="text-error hover:text-error"
                        ...
                        <!-- Assignment form -->
                        <div v-if="assignmentSection === section.id"
                            class="mt-3 rounded-md border border-accent/20 bg-accent/5 p-3">
                            <div class="grid gap-2 sm:grid-cols-[1fr_170px_auto]">
                                ...
                                <div class="flex gap-2">
                                    <BaseButton variant="secondary" @click="cancelAssignment">
                                        Cancel
                                    </BaseButton>
            ...
            <!-- Footer status -->
            <div v-if="isDraft" class="rounded-md border border-border bg-bg/40 p-3 text-xs text-text/50">
                Review the automatically selected sections and assign the instructors responsible
                for each section before approval.
            </div>
        </div>
    </BaseDialog>
    ...
    <BaseDialog :model-value="showRejectModal" title="Reject this offering?" max-width="max-w-md"
        ...
        <template #footer>
            <div class="flex justify-end gap-2">
                ...
                <BaseButton variant="danger" :disabled="!rejectReason.trim()" :loading="busy" @click="confirmReject">
                    Reject offering
                </BaseButton>
            </div>
        </template>
