...
const visibleColumns = ref<ProgramColumn[]>([
    'program',
    'code',
    'department',
    'duration',
...
const isColumnVisible = (column: ProgramColumn) =>
...
const { form, errors, validate, reset, applyServerErrors } =
    useResourceForm(programSchema, {
        department_id: 0,
        code: '',
        name: '',
        duration_years: 4,
...
function retry() {
    crud.load(
        crud.currentPagination().current_page,
    )
}
...
function handleDocumentClick(event: MouseEvent) {
    ...
    if (
        !target.closest('[data-columns-container]')
    ) {
...
<template>
    <div class="mx-auto w-full max-w-360 space-y-6 px-6 py-6">
        ...
        <div data-columns-container class="relative">
            <ResourceToolbar title="Programs" description="Manage academic programs under each department."
                search-placeholder="Search programs..." :show-search="true" :show-filter="true" :show-refresh="true"
                :show-columns="true" :show-fullscreen="true" :show-tabs="true" :active-tab="crud.activeTab.value"
                :active-count="crud.activePagination.value.total" :archived-count="crud.archivedPagination.value.total"
                :has-active-filters="hasActiveFilters" @clear-filters="clearFilters" @change-tab="crud.changeTab"
                @update:search="search = $event" @refresh="retry" @columns="showColumns = !showColumns">
                <template #actions>
                    <BaseButton v-can="'program.create'" :icon="Plus" @click="openCreate">
                        Add program
                    </BaseButton>
                </template>
                ...
                <template #filters>
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <BaseSelect v-model="filters.department_id" label="Department"
                            :options="departmentFilterOptions" placeholder="All departments" />
                    </div>
                </template>
            </ResourceToolbar>
                ...
                <div class="flex items-center justify-between px-2 py-2">
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-text/45">
                            Choose what appears in the table
                        </p>
                    </div>
        ...
        <div v-if="crud.error.value" class="flex items-start gap-3 rounded-md border border-error/30 bg-error/5 p-4">
            ...
            <button type="button" class="text-sm font-medium text-error hover:underline" @click="retry">
                Retry
            </button>
        </div>
                ...
                <table class="w-full min-w-190 border-collapse">
                    <thead>
                        <tr class="border-b border-border bg-text/2.5">
                            <th v-if="isColumnVisible('program')"
                                class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-text/50">
                                Program
                            </th>

                            <th v-if="isColumnVisible('code')"
                                class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-text/50">
                                Code
                            </th>

                            <th v-if="isColumnVisible('department')"
                                class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-text/50">
                                Department
                            </th>
                            ...
                            <th v-if="isColumnVisible('created_at')"
                                class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-text/50">
                                Created
                            </th>
                    ...
                    <tbody v-if="crud.loading.value" class="divide-y divide-border">
                        <tr v-for="row in 6" :key="row">
                            <td v-for="column in visibleColumns" :key="column" class="px-4 py-4">
                                <div class="h-3.5 animate-pulse rounded bg-text/5" :class="column === 'program'
                                        ? 'w-44'
                                        : column === 'department'
                                            ? 'w-32'
                                            : column === 'code'
                    ...
                    <tbody v-else-if="crud.currentList().length" class="divide-y divide-border">
                        <tr v-for="program in crud.currentList()" :key="program.id"
                            ...
                            <td class="px-4 py-3.5 text-right">
                                <TableRowActions :active-tab="crud.activeTab.value" edit-permission="program.update"
                                    archive-permission="program.archive" restore-permission="program.restore"
                                    @edit="openEdit(program)" @archive="openArchive(program)"
                            ...
                            <td :colspan="totalTableColumns" class="px-6 py-16 text-center">
                                <div class="mx-auto flex max-w-sm flex-col items-center">
                                    ...
                                    <p v-if="
                                        search ||
                                        hasActiveFilters
                                    " class="mt-1 text-xs text-text/45">
    ...
    <BaseDialog :model-value="showFormModal" :title="selected
            ? 'Edit program'
            : 'Add program'
        " @update:model-value="closeFormModal">
        <div class="space-y-4">
            <BaseSelect v-model="form.department_id" label="Department" :options="departmentOptions"
            ...
            <BaseInput v-model="form.name" label="Program name" placeholder="e.g. Software Engineering"
                :error="errors.name" />

            <BaseInput v-model.number="form.duration_years" type="number" label="Duration (years)" placeholder="4"
                :error="errors.duration_years" />
        </div>

        <template #footer>
            <div class="flex justify-end gap-2">
                <BaseButton variant="secondary" @click="closeFormModal">
                    Cancel
                </BaseButton>
    ...
    <ConfirmModal :show="showArchiveModal" title="Archive program?"
        :description="`This will remove ${selected?.name} from active program views.`" confirm-text="Archive program"
        variant="danger" :icon="Archive" :loading="archiving" @close="showArchiveModal = false"
        @confirm="confirmArchive" />

    <ConfirmModal :show="showRestoreModal" title="Restore program?"
        :description="`This will return ${selected?.name} to the active program list.`" confirm-text="Restore program"
