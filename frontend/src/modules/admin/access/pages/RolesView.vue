...
function openCreate() { editing.value = null; reset({ name: '', description: '' }); showFormModal.value = true }
function openEdit(role: Role) { editing.value = role; reset({ name: role.name, description: role.description }); showFormModal.value = true }
function closeFormModal() { showFormModal.value = false; editing.value = null }
...
function openDelete(role: Role) { editing.value = role; showDeleteModal.value = true }
async function confirmDelete() {
    ...
    try {
        ...
        if (selectedRoleId.value === editing.value.id) selectedRoleId.value = null
        await load()
    } catch (err: any) {
        deleting.value = false
...
<template>
    <div class="mx-auto flex w-full max-w-360 flex-col xl:flex-row gap-6 px-4 sm:px-6 py-6 min-h-screen">
        ...
        <PermissionEditor v-if="selectedRole" :role="selectedRole" class="w-full xl:w-120 shrink-0 xl:sticky xl:top-6"
    ...
    <ConfirmModal :show="showDeleteModal" title="Delete this role?"
        :description="`This will delete ${editing?.name}. If any users currently hold this role, deletion may be blocked.`"
        confirm-text="Delete role" variant="danger" :icon="Archive" :loading="deleting" @close="showDeleteModal = false"
