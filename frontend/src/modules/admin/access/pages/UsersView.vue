...
async function submitForm() {
    ...
    if (!result.success) { validate(); return }
    saving.value = true
    try {
        if (selected.value) {
...
<template>
                        ...
                        <tr v-for="user in users" :key="user.id" class="group hover:bg-text/2">
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-accent/10 text-xs font-bold text-accent">
                                        {{ initials(user) }}</div>
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-medium text-text">{{ user.first_name }} {{
                                            user.last_name }}</p>
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex flex-wrap gap-1">
                                    <BaseBadge v-for="r in user.role_name" :key="r" variant="neutral"
                                        class="text-[10px]">{{ r
                                        }}</BaseBadge>
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-1.5 text-xs">
                                    <span class="h-2 w-2 rounded-full"
                                        :class="user.is_active === false ? 'bg-text/30' : 'bg-success'" />
                            ...
                            <td class="px-4 py-3.5 text-right">
                                ...
                                <Teleport to="body">
                                    <div v-if="openMenuId === user.id" data-action-menu
                                        ...
                                        <button v-if="user.is_active !== false" type="button"
                                        ...
                                        <button v-else type="button"
                                            class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-success hover:bg-success/5"
                                            @click="openToggleStatus(user)">
    ...
    <BaseDialog :model-value="showFormModal" :title="isEditing ? 'Edit user' : 'Add user'"
        ...
        <div class="space-y-4">
            ...
            <BaseInput v-if="!isEditing" v-model="form.password" type="password" label="Temporary password"
                :error="errors.password" />
        </div>
        <template #footer>
            <div class="flex justify-end gap-2">
                <BaseButton variant="secondary" @click="closeFormModal">Cancel</BaseButton>
                <BaseButton :loading="saving" @click="submitForm">{{ isEditing ? 'Save changes' : 'Create user' }}
                </BaseButton>
            </div>
        </template>
    </BaseDialog>
    ...
    <ConfirmModal :show="showDisableModal"
        ...
        :confirm-text="selected?.is_active === false ? 'Activate' : 'Disable'" variant="danger" :icon="Ban"
        :loading="toggling" @close="showDisableModal = false" @confirm="confirmToggleStatus" />
</template>
