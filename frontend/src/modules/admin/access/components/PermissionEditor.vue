  ...
  <div class="flex max-h-[calc(100vh-2rem)] xl:max-h-[calc(100vh-4rem)] flex-col rounded-lg border border-border bg-surface shadow-sm">
    ...
    <div class="shrink-0 border-b border-border bg-bg/50 px-5 py-4">
      <div class="flex items-center justify-between">
        ...
        <span class="text-xs font-semibold text-text/50 bg-surface border border-border rounded-full px-2.5 py-1">
    ...
    <div v-else class="flex-1 space-y-3 overflow-y-auto p-4 min-h-0">
      <div v-for="[category, perms] in grouped" :key="category"
        class="overflow-hidden rounded-lg border border-border transition-colors"
        :class="isAllSelected(perms) ? 'border-accent/30' : ''">
        <button
          type="button"
          class="w-full flex items-center justify-between bg-bg px-4 py-3 hover:bg-bg/70 transition-colors"
        ...
        <div v-if="openCategories.has(category)" class="divide-y divide-border bg-surface">
          <label
            ...
            class="flex cursor-pointer items-center justify-between gap-3 px-4 py-3 hover:bg-bg/50 transition-colors"
          >
            <div class="min-w-0">
            ...
            <input
              ...
              class="h-4 w-4 shrink-0 rounded accent-accent cursor-pointer"
              @change="toggle(p.id)"
            />
          </label>
        </div>
