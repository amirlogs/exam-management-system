...
const {
    ...
    reset,
    applyServerErrors,
} = useResourceForm(departmentSchema, {
    college_id: null as number | null,
    name: '',
...
<template>
    <div class="mx-auto w-full max-w-360 space-y-6 px-6 py-6">
        ...
        <!-- TOOLBAR -->
        <ResourceToolbar title="Departments" description="Manage departments under each college."
            search-placeholder="Search departments..." :show-search="true" :show-filter="true" :show-refresh="true"
            :show-fullscreen="true" :show-tabs="true" :active-tab="crud.activeTab.value"
            :active-count="crud.activePagination.value.total" :archived-count="crud.archivedPagination.value.total"
            :has-active-filters="hasActiveFilters" :filter-count="filterCount" @clear-filters="clearFilters"
            @change-tab="crud.changeTab" @update:search="search = $event" @refresh="retry">
            <!-- Primary Action -->
            <template #actions>
                <BaseButton v-can="'department.create'" :icon="Plus" @click="openCreate">
                    Add department
                </BaseButton>
            </template>
            ...
            <template #filters>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <BaseSelect v-model="filters.college_id" label="College" :options="collegeFilterOptions"
                        placeholder="All colleges" />

                    <BaseSelect v-model="filters.type" label="Type" :options="typeFilterOptions"
            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-190 border-collapse">
                    ...
                    <tbody v-else-if="crud.currentList().length" class="divide-y divide-border">
                        <tr v-for="dept in crud.currentList()" :key="dept.id"
                            ...
                            <td class="px-4 py-3.5 text-right">
                                <TableRowActions :active-tab="crud.activeTab.value" edit-permission="department.update"
                                    archive-permission="department.archive" restore-permission="department.restore"
                                    @edit="openEdit(dept)" @archive="openArchive(dept)" @restore="openRestore(dept)" />
                            </td>
                        </tr>
                    </tbody>
    ...
    <BaseDialog :model-value="showFormModal" :title="selected ? 'Edit department' : 'Add department'"
        ...
        <div class="space-y-4">
            ...
            <BaseSelect v-model="form.college_id" label="College" :options="collegeOptions" placeholder="Select college"
