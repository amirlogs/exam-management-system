...
import { Archive, Calendar, Edit, Eye, Lock, LockOpen, Plus, RotateCcw } from 'lucide-vue-next'
...
const { form, errors, validate, reset } = useResourceForm(semesterSchema, {
...
function semesterTitle(s: Semester) {
    return `Semester ${s.name} · ${s.academic_year}`
}
...
async function submitForm() {
    ...
    try {
        ...
        saving.value = false
        handleApiError(err, uiStore, undefined, isEditing ? 'Failed to update semester.' : 'Failed to save semester.')
    }
}
...
function openArchive(s: Semester) { selected.value = s; showArchiveModal.value = true }
...
<template>
    <div class="mx-auto w-full max-w-260 space-y-8 px-6 py-6">
        <section class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-text/60">Manage academic terms and the currently open semester.</p>
            </div>
        ...
        <div v-if="!showArchived && openSemesterItem"
            ...
            <div class="flex flex-col justify-between gap-6 md:flex-row md:items-start">
                <div class="flex-1">
                    <div class="mb-3 flex items-center gap-3">
                        <span
                            ...
                            <span class="text-xs font-bold uppercase tracking-wide text-success">Currently open</span>
                        </span>
                ...
                <BaseButton :loading="togglingId === openSemesterItem.id" variant="secondary"
                    @click="toggleOpen(openSemesterItem)">
                    <template #icon>
                        <Lock class="h-4 w-4" />
                    </template>
                    Close semester
        ...
        <section>
            <div class="mb-4 flex items-center justify-between">
                <h2 class="text-lg font-semibold text-text">
                    {{ showArchived ? 'Archived semesters' : 'Past & upcoming semesters' }}</h2>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-1">
                                    <template v-if="!showArchived">
                                        <button type="button"
                                            class="rounded p-1.5 text-text/50 hover:bg-text/5 hover:text-text"
                                            title="Edit" @click="openEdit(s)">
                                            <Edit class="h-4 w-4" />
                                        </button>
                                        <button type="button"
                                            class="rounded p-1.5 text-text/50 hover:bg-text/5 hover:text-text"
                                            :title="s.status === 'open' ? 'Close' : 'Open'" @click="toggleOpen(s)">
                                    ...
                                    <button v-else type="button"
                                        class="rounded p-1.5 text-text/50 hover:bg-accent/10 hover:text-accent"
                                        title="Restore" @click="restoreOne(s)">
    ...
    <ConfirmModal :show="showToggleModal"
        :title="semesterToToggle?.status === 'active' ? 'Close this semester?' : 'Open this semester?'" :description="semesterToToggle?.status === 'active'
            ...
            : 'This will make this the active semester system-wide. Only one semester can be active at a time.'"
        :confirm-text="semesterToToggle?.status === 'active' ? 'Close semester' : 'Open semester'" variant="danger"
        :icon="semesterToToggle?.status === 'active' ? Lock : LockOpen" :loading="togglingId === semesterToToggle?.id"
