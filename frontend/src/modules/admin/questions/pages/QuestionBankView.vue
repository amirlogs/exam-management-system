...
const crud = useCrudResource<any>(
  {
    ...
    restore: async (q) => { const r = await restoreQuestion(q.id); return r },
  },
  'content',
)
...
const typeOptions = [
  ...
  { value: 'mcq', label: 'MCQ' }, { value: 'true_false', label: 'True/False' },
  { value: 'short_answer', label: 'Short Answer' }, { value: 'essay', label: 'Essay' },
]
const difficultyOptions = [{ value: null, label: 'Any difficulty' }, { value: 'easy', label: 'Easy' }, { value: 'medium', label: 'Medium' }, { value: 'hard', label: 'Hard' }]
...
function clearFilters() { search.value = ''; filters.value = { course_id: null, type: null, difficulty: null } }
...
function openArchive(q: any) { selected.value = q; showArchiveModal.value = true }
function openRestore(q: any) { selected.value = q; showRestoreModal.value = true }
async function confirmArchive() {
  if (!selected.value) return
  archiving.value = true
  try { await crud.archive(selected.value); showArchiveModal.value = false; selected.value = null }
  catch { uiStore.showToast('Failed to archive question.', 'error') }
...
onMounted(async () => { await loadCourses(); await crud.load(1) })
</script>
...
<template>
  <div class="mx-auto w-full max-w-360 space-y-6 px-6 py-6">
    <ResourceToolbar
      ...
      :show-search="true" :show-filter="true" :show-refresh="true" :show-tabs="true"
      :active-tab="crud.activeTab.value" :active-count="crud.activePagination.value.total" :archived-count="crud.archivedPagination.value.total"
      :has-active-filters="hasActiveFilters" :refreshing="crud.loading.value"
      @change-tab="crud.changeTab" @update:search="search = $event" @refresh="retry" @clear-filters="clearFilters"
    >
      <template #actions>
      ...
      <div class="overflow-x-auto">
        <table class="w-full min-w-190 border-collapse">
          ...
          <tbody v-if="crud.loading.value" class="divide-y divide-border">
            <tr v-for="row in 6" :key="row"><td colspan="5" class="px-4 py-4"><div class="h-3.5 w-full max-w-sm animate-pulse rounded bg-text/5" /></td></tr>
          </tbody>
          <tbody v-else-if="crud.currentList().length" class="divide-y divide-border">
            <tr v-for="q in crud.currentList()" :key="q.id" class="hover:bg-text/2">
              ...
              <td class="px-4 py-3.5"><BaseBadge :variant="typeBadge[q.type] || 'neutral'" class="uppercase text-[10px]">{{ q.type }}</BaseBadge></td>
              ...
              <td class="px-4 py-3.5 text-right">
                <TableRowActions
                  ...
                  @archive="openArchive(q)" @restore="openRestore(q)"
                />
              </td>
            </tr>
          </tbody>
          <tbody v-else><tr><td colspan="5" class="px-6 py-16 text-center text-sm text-text/55">No questions found.</td></tr></tbody>
        </table>
      </div>
  ...
  <ConfirmModal :show="showArchiveModal" title="Archive question?" description="This question will be removed from active views." confirm-text="Archive question" variant="danger" :icon="Archive" :loading="archiving" @close="showArchiveModal = false" @confirm="confirmArchive" />
  <ConfirmModal :show="showRestoreModal" title="Restore question?" description="This question will return to active views." confirm-text="Restore question" variant="accent" :icon="RotateCcw" :loading="restoring" @close="showRestoreModal = false" @confirm="confirmRestore" />
</template>
