...
const selectedCurriculum = computed(() =>
    curricula.value.find(
        (curriculum) =>
            curriculum.id === selectedCurriculumId.value,
    ) ?? null,
)
...
function openEditVersionModal() {
    ...
    resetEditVersionForm({
        program_id:
            selectedCurriculum.value.program_id,
        version:
            selectedCurriculum.value.version,
    })
...
const years = computed(() => {
    const duration =
        Number(
            selectedProgram.value?.duration_years,
        ) || 4
...
function coursesFor(
    ...
    return (
        selectedCurriculum.value?.courses.filter(
            (course) =>
                course.year_level === year &&
                course.semester_number === semester,
        ) ?? []
    )
}
...
const courseToRemove =
    ref<CurriculumCourse | null>(null)
...
async function confirmRemove() {
    ...
    try {
        await removeCurriculumCourse(
            courseToRemove.value.id,
        )
...
<template>
    ...
    >
        ...
        >
            ...
            >
                ...
                <div class="min-w-0">
                    <h1
                        class="font-display text-2xl font-bold text-text"
                    >
                        Curriculum
                    </h1>
            ...
            <div
                ...
                class="mt-5 flex flex-col gap-4 border-t border-border pt-4 lg:flex-row lg:items-center lg:justify-between"
            >
                <!-- VERSION SELECTOR -->
                <div
                ...
                >
                    <span
                        class="mr-1 text-xs font-bold uppercase tracking-wide text-text/50"
                    >
                        Version
                    </span>

                    <div
                        class="flex flex-wrap items-center gap-1 rounded-lg border border-border bg-bg p-1"
                    >
                        <button
        ...
        <div
            v-if="loadingCurricula"
            class="rounded-md border border-border bg-surface py-16 text-center"
        >
            <RefreshCw
                class="mx-auto h-6 w-6 animate-spin text-text/30"
            />

            <p
                class="mt-3 text-sm text-text/50"
            >
                Loading curriculum...
            </p>
        </div>
        ...
        >
            <BookOpen
                class="mb-3 h-8 w-8 text-text/30"
            />
            ...
            <p
                class="mt-1 text-sm text-text/55"
            >
                Create the first version for this
                program.
            </p>
        ...
        >
            ...
            >
                <!-- YEAR -->
                <div
                    class="mb-4 flex items-center justify-between border-l-4 border-border pl-3"
                >
                    <h2
                    ...
                    <span
                        class="rounded bg-bg px-2 py-1 font-mono text-xs text-text/50"
                    >
                        {{
                            creditsForYear(
                                year,
                ...
                >
                    <div
                        ...
                        :key="semester"
                        class="flex min-h-55 flex-col rounded-md border border-border bg-surface p-4"
                    >
                        <div
                            class="mb-3 flex items-center justify-between border-b border-border pb-3"
                        >
                            <h3
                            ...
                            <span
                                class="font-mono text-xs text-text/50"
                            >
                                {{
                                    creditsFor(
                                        year,
                        ...
                        >
                            <div
                                v-for="cc in coursesFor(
                                    year,
                                    semester,
                                )"
                                :key="cc.id"
                                class="flex items-center justify-between rounded-md border border-border bg-bg px-3 py-2.5"
                            >
                                <div
                                    class="min-w-0"
                                >
                                    <p
                                        class="truncate font-mono text-xs font-semibold text-text"
                                    >
                                        {{
                                            cc.course
                                                ?.code ??
                                    ...
                                    <p
                                        class="truncate text-xs text-text/55"
                                    >
                                        {{
                                            cc.course
                                                ?.name ??
                                ...
                                >
                                    <span
                                        v-if="
                                            ...
                                            null
                                        "
                                        class="rounded bg-surface px-2 py-0.5 font-mono text-xs text-text/60"
                                    >
                                        {{
                                            cc.course
                                                .credit_hours
    ...
    <ConfirmModal
        ...
        title="Remove course?"
        :description="
            `This will remove ${courseToRemove?.course?.code ?? 'this course'} from this curriculum. This cannot be undone.`
        "
