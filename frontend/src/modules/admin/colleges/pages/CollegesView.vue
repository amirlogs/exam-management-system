...
import { Archive, Building, Edit, MoreVertical, Plus, RefreshCw, RotateCcw, Search, X } from 'lucide-vue-next'
...
<template>
    <div class="mx-auto w-full max-w-360 space-y-6 px-6 py-6">
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h1 class="text-2xl font-semibold tracking-tight text-text">Colleges</h1>
                <p class="mt-1 text-sm text-text/60">Manage the colleges in your institution.</p>
            </div>
            <BaseButton @click="openCreate"><template #icon><Plus class="h-4 w-4" /></template>Add college</BaseButton>
        </section>
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="relative w-full max-w-md">
                ...
                <input v-model="search" type="search" placeholder="Search colleges..." class="h-9 w-full rounded-md border border-border bg-surface pl-9 pr-9 text-sm text-text outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/20" />
                ...
                <table class="w-full min-w-140 border-collapse">
                    ...
                    <tbody v-if="crud.loading.value" class="divide-y divide-border">
                        <tr v-for="row in 6" :key="row">
                            <td class="px-4 py-4"><div class="flex items-center gap-3"><div class="h-9 w-9 animate-pulse rounded-md bg-text/5" /><div class="h-3.5 w-44 animate-pulse rounded bg-text/5" /></div></td>
                            <td class="px-4 py-4"><div class="h-3.5 w-24 animate-pulse rounded bg-text/5" /></td>
                            <td />
                        </tr>
                    </tbody>
                        ...
                        <tr v-for="college in filteredList" :key="college.id" class="group transition-colors hover:bg-text/2">
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-3">
                                    <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-border bg-bg text-text/50"><Building class="h-4 w-4" /></div>
                                    <p class="text-sm font-medium text-text">{{ college.name }}</p>
                                </div>
                            </td>
                            <td class="px-4 py-3.5"><span class="font-mono text-xs tabular-nums text-text/60">{{ college.created_at }}</span></td>
                            <td class="px-4 py-3.5 text-right">
                                <button type="button" data-action-trigger class="inline-flex h-8 w-8 items-center justify-center rounded-md text-text/50 transition hover:bg-text/5 hover:text-text" @click.stop="toggleMenu(college.id, $event)"><MoreVertical class="h-4 w-4" /></button>
                                <Teleport to="body">
                                    <div v-if="openMenuId === college.id" data-action-menu class="fixed z-[100] w-40 rounded-md border border-border bg-surface py-1 shadow-lg" :style="{ top: menuPosition.top + 'px', left: menuPosition.left + 'px' }" @click.stop>
                                        <button v-if="crud.activeTab.value === 'active'" type="button" class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-text hover:bg-text/5" @click="openEdit(college)"><Edit class="h-4 w-4 text-text/50" /> Edit</button>
                                        <div v-if="crud.activeTab.value === 'active'" class="my-1 border-t border-border" />
                                        <button v-if="crud.activeTab.value === 'active'" type="button" class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-error hover:bg-error/5" @click="openArchive(college)"><Archive class="h-4 w-4" /> Archive</button>
                                        <button v-else type="button" class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-accent hover:bg-accent/5" @click="openRestore(college)"><RotateCcw class="h-4 w-4" /> Restore</button>
                                    </div>
                                </Teleport>
                            </td>
                        </tr>
                    ...
                    <tbody v-else><tr><td colspan="3" class="px-6 py-16 text-center text-sm text-text/55">{{ emptyStateText }}</td></tr></tbody>
                </table>
            </div>
    ...
    <BaseDialog :model-value="showFormModal" :title="selected ? 'Edit college' : 'Add college'" @update:model-value="closeFormModal">
        <BaseInput v-model="form.name" label="College name" placeholder="e.g. College of Engineering" :error="errors.name" />
        <template #footer>
            <div class="flex justify-end gap-2">
                <BaseButton variant="secondary" @click="closeFormModal">Cancel</BaseButton>
                <BaseButton :loading="saving" @click="submitForm">{{ selected ? 'Save changes' : 'Create college' }}</BaseButton>
            </div>
        </template>
    </BaseDialog>

    <ConfirmModal :show="showArchiveModal" title="Archive college?" :description="`This will remove ${selected?.name} from active college views. It can be restored later.`" confirm-text="Archive college" variant="danger" :icon="Archive" :loading="archiving" @close="showArchiveModal = false" @confirm="confirmArchive" />
    <ConfirmModal :show="showRestoreModal" title="Restore college?" :description="`This will return ${selected?.name} to the active college list.`" confirm-text="Restore college" variant="accent" :icon="RotateCcw" :loading="restoring" @close="showRestoreModal = false" @confirm="confirmRestore" />
</template>
