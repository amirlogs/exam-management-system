...
export async function getCourses(page = 1, perPage = 10): Promise<ListResponse<Course>> {
  const res = await api.get('/courses', { params: { page, per_page: perPage } })
  return res.data
}
export async function getArchivedCourses(page = 1, perPage = 10): Promise<ListResponse<Course>> {
  const res = await api.get('/courses/archived', { params: { page, per_page: perPage } })
  return res.data
}
