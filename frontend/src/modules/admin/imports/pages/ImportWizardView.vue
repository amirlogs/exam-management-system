...
async function confirmFinal() {
    ...
    } catch { /* toast already shown by composable */ }
}
    ...
    <div class="mx-auto w-full max-w-260 space-y-6 px-6 py-6">
        <div>
            ...
            <h1 class="mt-1 text-2xl font-semibold tracking-tight text-text">
                {{ !record ? 'Upload file' : record.status === 'ready_for_review' ? 'Review data' : record.status ===
                    'confirmed' ? 'Import complete' : 'Processing' }}
            </h1>
        </div>

        <ImportUploadStep v-if="!record" :type="type" :uploading="isUploading" :error="uploadError"
            @upload="handleUpload" />

        <ImportProcessingStep v-else-if="record.status === 'pending' || record.status === 'processing'"
            :total-rows="record.total_rows" />

        <div v-else-if="record.status === 'ready_for_review'" class="space-y-6">
            <ImportReviewTable :rows="record.validated_data ?? {}" :saving-row="savingRow"
        ...
        <div v-else-if="record.status === 'confirmed'"
            class="flex flex-col items-center rounded-md border border-border bg-surface p-12 text-center">
            <CheckCircle2 class="mb-4 h-10 w-10 text-success" />
            <h3 class="mb-1 font-semibold text-text">Import confirmed</h3>
            <p class="text-sm text-text/60">{{ record.valid_count }} {{ config.label.toLowerCase() }} added
                successfully.</p>
        </div>
    </div>
