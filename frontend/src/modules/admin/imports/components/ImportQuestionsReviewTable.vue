  ...
  <div class="overflow-hidden rounded-md border border-border bg-surface">
    <div class="flex items-center justify-between border-b border-border bg-bg/50 px-5 py-3">
      <div class="flex items-center gap-3 text-xs">
        <span class="font-semibold uppercase tracking-wide text-text/60">{{ Object.keys(rows).length }} questions</span>
      ...
      <div v-for="[rowKey, row] in entries" :key="rowKey" :class="row.status === 'invalid' ? 'bg-error/5' : ''">
        ...
        <div class="flex cursor-pointer items-center justify-between gap-4 p-4 hover:bg-text/2" @click="toggleExpand(rowKey)">
          ...
          <div class="flex shrink-0 items-center gap-2" @click.stop>
            <BaseBadge variant="neutral" class="text-[10px] uppercase">{{ row.data.type || 'unknown' }} </BaseBadge>
            <BaseBadge :variant="row.status === 'valid' ? 'success' : 'danger'">
              {{ row.status === 'valid' ? 'Valid' : `${row.errors.length} issue(s)` }}
            </BaseBadge>
            <template v-if="!readonly">
              <button
                v-if="editingRow !== rowKey"
                type="button"
        ...
        <div v-if="expandedRow === rowKey" class="border-t border-border bg-bg/20 p-5">
          <div v-if="row.errors.length" class="mb-4 rounded-md border border-error/20 bg-error/10 p-3 text-xs">
          ...
          <div v-if="editingRow !== rowKey" class="space-y-4">
            <div class="grid grid-cols-1 gap-4 border-b border-border/50 pb-3 text-xs md:grid-cols-3">
              <div>
                <span class="text-text/50">Difficulty:</span> <span class="ml-1.5 font-semibold uppercase">{{ row.data.difficulty || 'N/A' }}</span>
              </div>
              <div>
                <span class="text-text/50">Chapter:</span> <span class="ml-1.5 font-semibold">{{ row.data.chapter || 'N/A' }}</span>
              </div>
              <div v-if="!isChoiceType(row.data.type)">
                <span class="text-text/50">Expected answer:</span> <span class="ml-1.5 font-semibold text-success">{{ row.data.correct_answer || 'N/A' }}</span>
              </div>
            </div>
            ...
            <div v-if="isChoiceType(row.data.type)" class="space-y-2">
              ...
              <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                <div
                  v-for="(opt, idx) in parsedOptions(row.data.options)"
                  :key="idx"
          ...
          <div v-else class="space-y-4 rounded-md border border-accent/30 bg-surface p-5">
            ...
            <div class="grid grid-cols-1 gap-4 text-xs md:grid-cols-3">
              ...
              <div>
                ...
                <select v-model="draft.type" class="w-full rounded-md border border-border bg-bg px-3 py-2 text-sm">
              ...
              <div>
                ...
                <select v-model="draft.difficulty" class="w-full rounded-md border border-border bg-bg px-3 py-2 text-sm uppercase">
              ...
              <div>
                ...
                <input v-model="draft.chapter" type="text" class="w-full rounded-md border border-border bg-bg px-3 py-2 text-sm" />
              </div>
              ...
              <div v-if="isChoiceType(draft.type)" class="space-y-3 pt-2 md:col-span-3">
                <div class="flex items-center justify-between">
                  ...
                  <button v-if="draft.type === 'MCQ'" type="button" class="inline-flex items-center gap-1 text-xs font-semibold text-accent" @click="addOption">
                    <Plus class="h-3.5 w-3.5" /> Add choice
                  </button>
                </div>
                <div class="space-y-2">
                  <div v-for="(_, idx) in draft.options" :key="idx" class="flex items-center gap-2">
                    <input type="radio" :checked="draft.correct_answer === draft.options[idx]" class="h-4 w-4 accent-accent" @change="draft.correct_answer = draft.options[idx]" />
                    <input v-model="draft.options[idx]" type="text" placeholder="Option text..." class="flex-1 rounded-md border border-border bg-bg px-3 py-1.5 text-xs" />
                    <button v-if="draft.type === 'MCQ' && draft.options.length > 2" type="button" class="p-1.5 text-text/40 hover:text-error" @click="removeOption(idx)">
              ...
              <div v-else class="pt-2 md:col-span-3">
                ...
                <textarea v-model="draft.correct_answer" rows="2" class="w-full rounded-md border border-border bg-bg px-3 py-2 text-xs" />
              </div>
            </div>

            <div class="flex items-center justify-end gap-2 border-t border-border pt-3">
              <BaseButton variant="secondary" @click="cancelEdit"
                ><template #icon> <X class="h-4 w-4" /> </template>Cancel
              </BaseButton>
              <BaseButton :loading="savingRow === rowKey" @click="saveEdit(rowKey)"
                ><template #icon> <Check class="h-4 w-4" /> </template>Save row</BaseButton
              >
            </div>
          </div>
        </div>
