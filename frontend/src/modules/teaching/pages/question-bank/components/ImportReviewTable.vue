...
function saveEdit(row: QuestionRow) {
  ...
  const payload: QuestionRowData = {
    ...draft.value,
    options: isChoiceType ? opts.map(o => o.trim()).filter(Boolean) : undefined,
  }
  ...
  <BaseCard :padded="false" class="overflow-hidden shadow-sm border border-border">
    <div class="px-5 py-3.5 bg-bg/50 border-b border-border flex items-center justify-between">
      <div class="flex items-center gap-3">
        <span class="text-xs font-semibold uppercase tracking-wider text-text/60">Total: {{ rows.length }} Questions</span>
        <span class="text-border">|</span>
        <span class="text-xs font-medium text-emerald-600">{{ rows.filter(r => r.status === 'valid').length }} Valid</span>
        <span class="text-xs font-medium text-red-600">{{ rows.filter(r => r.status === 'invalid').length }} Invalid</span>
      </div>
      <button
        type="button"
    ...
    <div class="divide-y divide-border">
      <div v-for="row in (filterInvalidOnly ? rows.filter(r => r.status === 'invalid') : rows)" :key="row.row" class="bg-surface">
        <div
          class="p-4 flex items-center justify-between gap-4 cursor-pointer select-none hover:bg-bg/40"
        ...
        >
          ...
          <div class="flex items-center gap-3 shrink-0" @click.stop>
            <BaseBadge variant="neutral" class="uppercase text-[10px] tracking-wide">{{ row.data.type || 'UNKNOWN' }}</BaseBadge>
            ...
            <button type="button" @click="toggleExpand(row.row)" class="p-1 text-text/40 hover:text-text rounded">
        ...
        <div v-if="expandedRow === row.row" class="p-5 bg-bg/20 border-b border-border">
          <div v-if="safeErrors(row).length > 0" class="mb-4 p-3.5 rounded-lg bg-red-500/10 border border-red-500/20 text-xs">
            <div class="font-bold flex items-center gap-1.5 text-red-800 mb-1"><AlertTriangle :size="14" /> Needs correction:</div>
          ...
          <div v-if="editingRow !== row.row" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 pb-3 border-b border-border/50 text-xs">
              <div><span class="text-text/50">Difficulty:</span> <span class="ml-1.5 font-semibold uppercase">{{ row.data.difficulty || 'N/A' }}</span></div>
              <div><span class="text-text/50">Points:</span> <span class="ml-1.5 font-semibold">{{ row.data.points ?? 1 }}</span></div>
              <div class="md:col-span-2"><span class="text-text/50">Correct Answer:</span> <span class="ml-1.5 font-semibold text-emerald-600">{{ row.data.correct_answer || 'N/A' }}</span></div>
            </div>
          ...
          <div v-else class="space-y-4 bg-surface p-5 rounded-lg border border-accent/30 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
              ...
              <div>
                ...
                <select v-model="draft!.difficulty" class="w-full px-3 py-2 rounded-md border border-border bg-bg text-sm uppercase">
              ...
              <div>
                ...
                <input v-model.number="draft!.points" type="number" step="0.5" class="w-full px-3 py-2 rounded-md border border-border bg-bg text-sm" />
              </div>

              <div v-if="draft!.type === 'mcq' || draft!.type === 'true_false'" class="md:col-span-3 space-y-3 pt-2">
                <div class="flex items-center justify-between">
                  <label class="font-semibold text-text/70">Choices & Correct Answer</label>
                  <button v-if="draft!.type === 'mcq'" type="button" @click="addOption" class="inline-flex items-center gap-1 text-xs text-accent font-semibold">
                    <Plus :size="13" /> Add Choice
                  </button>
                </div>
                <div class="space-y-2">
                  <div v-for="(_, optIdx) in draft!.options" :key="optIdx" class="flex items-center gap-2">
                    ...
                    <button v-if="draft!.type === 'mcq' && draft!.options.length > 2" type="button" @click="removeOption(optIdx)"
                      class="p-1.5 text-text/40 hover:text-red-500">
                      <Trash2 :size="14" />
                    </button>
                  </div>
                </div>
              ...
              <div v-else class="md:col-span-3 pt-2">
                ...
                <textarea v-model="draft!.correct_answer" rows="2" class="w-full px-3 py-2 rounded-md border border-border bg-bg text-xs" />
              </div>
            </div>
