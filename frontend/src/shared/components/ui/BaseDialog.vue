...
<template>
    <Teleport to="body">
        <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog"
            aria-modal="true" :aria-label="title || 'Dialog'">
            <!-- Backdrop -->
            <div class="absolute inset-0 bg-black/40" @click="handleBackdropClick" />

            <!-- Dialog -->
            <div class="relative flex max-h-[calc(100vh-2rem)] w-full flex-col overflow-hidden rounded-md border border-border bg-surface shadow-sm"
                :class="maxWidth" @click.stop>
                <!-- Header -->
