...
const filteredOptions = computed(() => (query.value ? props.options.filter((o) => o.label.toLowerCase().includes(query.value.toLowerCase())) : props.options));
...
<template>
  <div>
    <label v-if="label" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-text/70">{{ label }}</label>
    <button
      ref="triggerEl"
      type="button"
    ...
    <Teleport to="body">
      <div v-if="isOpen" data-combobox-menu class="fixed z-[100] flex max-h-72 flex-col rounded-lg border border-border bg-surface shadow-lg" :style="menuStyle" @click.stop>
        <div class="flex items-center gap-2 border-b border-border px-3 py-2">
          <Search class="h-4 w-4 shrink-0 text-text/40" />
          <input v-model="query" type="text" placeholder="Search..." autofocus class="w-full bg-transparent text-sm text-text outline-none placeholder:text-text/40" />
        </div>
