...
<template>
  <div ref="el" class="relative">
    <button type="button" @click="isOpen = !isOpen" class="w-9 h-9 rounded-full flex items-center justify-center hover:bg-bg transition-colors" title="Theme">
    ...
    <div v-if="isOpen" class="absolute right-0 mt-2 w-52 bg-surface border border-border rounded-lg shadow-lg py-1 z-20">
      ...
      <button v-for="t in themes" :key="t.id" @click="themeStore.setTheme(t.id)" class="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left hover:bg-bg transition-colors">
        <span class="w-3 h-3 rounded-full border border-border/50" :style="{ backgroundColor: t.swatch }" />
        <span class="flex-1" :class="themeStore.theme === t.id ? 'font-semibold text-text' : 'text-text/70'">{{ t.label }}</span>
