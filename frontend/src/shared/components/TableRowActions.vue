...
const activeExtras = computed(() => visibleExtras.value.filter((a) => !a.showOn || a.showOn === 'both' || a.showOn === 'active'));

const archivedExtras = computed(() => visibleExtras.value.filter((a) => !a.showOn || a.showOn === 'both' || a.showOn === 'archived'));
...
function handleExtraAction(key: string) {
  closeMenu();
  emit('action', key);
}
      ...
      <div
        ...
        <template v-if="activeTab === 'active'">
          ...
          <button
            ...
            @click="handleExtraAction(a.key)">
            <component :is="a.icon" class="h-4 w-4" :class="a.variant ? '' : 'text-text/50'" />
            {{ a.label }}
          </button>

          <div v-if="(editPermission || activeExtras.length) && archivePermission" class="my-1 border-t border-border" />

          <button
            v-if="archivePermission"
            v-can="archivePermission"
            type="button"
        ...
        <template v-else>
          <button
            v-if="restorePermission"
            v-can="restorePermission"
            type="button"
