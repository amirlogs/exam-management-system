...
<template>
  ...
  <div v-else class="relative">
    <button type="button" @click="isOpen = !isOpen" class="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium text-text hover:bg-bg">
      {{ currentLabel }}

      <ChevronDown class="w-4 h-4 text-text/50 transition-transform" :class="{ 'rotate-180': isOpen }" />
    </button>
    ...
    <div v-if="isOpen" class="absolute right-0 mt-2 w-52 bg-surface border border-border rounded-lg shadow-lg py-1 z-20">
      <button
        ...
        :class="workspace === currentWorkspace ? 'text-accent font-semibold bg-accent/5' : 'text-text'">
        {{ labels[workspace] }}

        <Check v-if="workspace === currentWorkspace" class="w-4 h-4" />
      </button>
    </div>
  </div>
</template>
