...
<template>
  ...
  <div v-if="visibleWorkspaces.length <= 1" class="h-9 px-3 rounded-xl border border-border/60 bg-bg/50 flex items-center text-xs font-mono text-text/60 select-none">
  ...
  <div v-else ref="el" class="relative">
    ...
    <Transition
      ...
      <div v-if="isOpen" class="absolute right-0 mt-2 w-52 bg-surface border border-border rounded-2xl shadow-xl p-1.5 z-30">
        <div class="px-2.5 py-1.5 text-[10px] font-mono font-semibold uppercase tracking-wider text-text/40">Switch Workspace</div>
        <div class="space-y-0.5">
          <button
            ...
            :class="workspace === currentWorkspace ? 'text-accent font-semibold bg-accent/10' : 'text-text hover:bg-bg hover:text-text'">
