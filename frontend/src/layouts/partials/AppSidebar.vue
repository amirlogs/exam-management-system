...
<template>
    ...
    <aside
        ...
        :class="[
            isCollapsed ? 'md:w-19' : 'md:w-66',
            isMobileOpen ? 'translate-x-0 w-70' : '-translate-x-full md:translate-x-0 w-70 md:w-auto',
        ]">
        <div class="overflow-y-auto flex-1 flex flex-col">
            <!-- Top bar -->
            <div class="h-17 px-3 flex items-center justify-between border-b border-border shrink-0"
                ...
                <button v-else-if="isCollapsed" @click="toggleCollapse"
            ...
            <nav class="p-4 space-y-1.5 flex-1">
                <router-link v-for="item in items" :key="item.to" :to="item.to" @click="handleNavigate"
        ...
        <div class="px-4 py-2 border-t border-border shrink-0">
            <router-link to="/settings" @click="handleNavigate"
